"""
Quirón — Knowledge Capture Agent
...
que la empresa no está dispuesta a asumir". Quirón extrae el conocimiento
...
QUESTIONS_PROMPT = """Sos Quirón, el agente de captura de conocimiento del sistema Mythos.
...
def generate_questions(topic: str, lang: str = "es") -> list[str]:
    ...
    if not preguntas:
        raise RuntimeError("Quirón no pudo generar preguntas. Probá reformular el tema.")
    return preguntas


# ── Función B: Síntesis del documento ─────────────────────────────────────────

SYNTH_PROMPT = """Sos Quirón, el agente de captura de conocimiento del sistema Mythos.
